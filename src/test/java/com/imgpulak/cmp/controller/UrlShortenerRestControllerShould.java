...
import com.imgpulak.cmp.CabManagementPortal;
...
public class UrlShortenerRestControllerShould {
  private static final String CLIENT_ID = new String("test");
...
/*
  ...
  private void sendPostRequestAndAssertResponse(URI uriToBeShortened, String expectedShortUri) throws Exception {
    String payload = "{\"clientId\":" + CLIENT_ID + ",\"urlStr\":" + uriToBeShortened.toString() + "}";
    mockMvc.perform(post("/")
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(payload)
    )
